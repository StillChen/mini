...
ZsSuit.prototype = {
    ...
    for (var i in this.suitRuleInfo) {
      ...
      for (var j in curSuitRuleInfo) {
        if (this.inArr(curSuitRuleInfo[j], this.paramsArr[j]) < 0) {
          ...
          if (curSuitRuleInfo[j] != '' || curSuitRuleInfo[j]>0) {
  ...
  out: function () {
    ...
    if (number == this.num) {
      ...
      curPattren = '^' + curPattren+'$';
      console.log(curPattren);
      for (var curSuitId in this.suitRuleInfo) {
        var pattren = new RegExp(curPattren.replace('(', '\\(').replace(')', '\\)'));
  ...
  jurdgeSuit: function (curPattren, positionArr) {
    ...
    for (var i in suitRuleInfo) {
      ...
      var flag = curVal.indexOf(curPattren);//pattren.test(curVal);
      if (flag>=0) {
